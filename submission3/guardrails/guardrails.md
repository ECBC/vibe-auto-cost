...
The cost engine enforces hard safety boundaries. Relevant code sections below.

## `backend/load_data.py` — cost engine + guardrails
```python
"""
...
from config import ANNUAL_KM, ELECTRICITY_RATE, USE_SAMPLE_DATA
...
def _load_sample():
    """Load synthetic sample data (no file dependencies)."""
    global european_records, european_by_brand, european_by_brand_model
    global global_median_fuel, global_median_maint, all_brands, models_by_brand, engines_by_brand_model
    global uci_records

    from sample_data import EUROPEAN_SAMPLE, UCI_SAMPLE

    for rec in EUROPEAN_SAMPLE:
        european_records.append(rec)
        brand_lower = rec["Brand"].strip().lower()
        european_by_brand.setdefault(brand_lower, []).append(rec)
        model_name = (rec.get("Model Name") or "").strip().lower()
...
## `backend/main.py` — API validation / 422 guardrail / structured logging
```python
"""
...
# ---------------------------------------------------------------------------
# Structured Logging (stdout, JSON-ish one-line format)
# ---------------------------------------------------------------------------
logger = logging.getLogger("vibe_auto_cost")
...
from load_data import load_all, calculate_evaluation, all_brands, models_by_brand, engines_by_brand_model  # noqa: E402
...
def evaluate(
    ...
    logger.info(
        '{"event":"evaluate","make":"%s","model":"%s","engine":"%s",'
        '"match_type":"%s","matched_records":%d,"elapsed_ms":%.2f}',
        make.strip(),
        model.strip(),
        engine.strip(),
        result["metadata"].get("_match_type", "unknown"),
        result["metadata"]["matched_records"],
        elapsed_ms,
    )
